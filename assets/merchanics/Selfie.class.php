...
class Selfie
{
    ...
    public static function add($People)
    {
        ...
        $Selfie->setVisible(1);
        $Selfie->setId(\ass\MAP::getInstance()->store('selfie', get_object_vars($Selfie)));
        if ($Selfie->getId() > 0)
            return ($Selfie);
        return (false);
    }
    ...
    {
        $Selfie = \ass\MAP::getInstance()->findOne('selfie', array('name' => $name, 'people' => $people));
        if ($Selfie instanceof Selfie) {
            $Selfie->setVisible(0);
            return \ass\MAP::getInstance()->store('selfie', get_object_vars($Selfie));
        }
        return false;
    }
